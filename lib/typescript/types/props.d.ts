...
type OmittedProps = 'onScroll' | 'scrollEventThrottle' | 'removeClippedSubviews' | 'CellRendererComponent' | 'numColumns';
export interface ReorderableListRenderItemInfo<T> {
    item: T;
    index: number;
}
export type ReorderableListRenderItem<T> = (info: ReorderableListRenderItemInfo<T>) => React.ReactElement | null;
