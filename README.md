...
| lockedIndices             | `number[]`                                       | No       | `[]`                       | Indices of items that are locked in place. A locked item can't be dragged, and a dragged item can't be moved across it, so the locked item never shifts. Indices are positional, referring to the item at that position in `data`. |
